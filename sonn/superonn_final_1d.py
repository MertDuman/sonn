...
def calculate_output_padding(n, k, p, s, d):
    """
    Calculates the output padding for a transposed convolution, such that Conv2d and ConvTranspose2d initialized to the same
    parameters will be inverses of each other in terms of input and output size.
    """
...
class SuperONNTranpose1d(nn.Module):
    """
    Parameters
    ----------
    in_channels : int
    ...
    stride : int
    dilation : int
    output_padding : int or str
        Same as ConvTranspose2d, except, if 'auto' the output padding is calculated such that we assume the input comes from a Conv2d layer with the
        same parameters as this layer. In that case, the padding will ensure that the output of this layer is the same size as the input of the Conv2d layer.
        You must pass expected_output_size, which is the size of the input of the Conv2d layer.
    expected_output_size : int
        The size of the input of the Conv2d layer that this layer is the inverse of. Only used when output_padding is 'auto'.
    groups : int or iterable of int or str
        Works the same as in nn.Conv2d, except (in_channels * q * full_groups) are split into groups (as opposed to in_channels).
        If groups > 1:
            in_channels * q must be divisible by (groups / full_groups), and groups must be divisible by full_groups.
    ...
    def __init__(
            ...
            stride: int = 1,
            dilation: int = 1,
            output_padding: Union[int, str] = 0,
        ...
        assert output_padding != 'auto' or expected_output_size is not None, "expected_output_size must be passed when output_padding is 'auto'"
        ...
        if isinstance(self.groups, Iterable):
            ...
            neuron_depth = (in_channels * self._impl_q * full_groups) // groups  # (in_channels * q * full_groups) / groups
            self.weight = nn.Parameter(torch.empty(neuron_depth, self.out_channels, *self.kernel_size, dtype=dtype))  # Q x C x K x D
            self.bias = nn.Parameter(torch.empty(self.out_channels)) if self.use_bias else self.register_parameter('bias', None)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self._iterable_groups:
            ...
            # TODO: Kernel size is a tuple, but padding, stride, etc are not.
            op = self.output_padding
            if self.output_padding == "auto":
                op = calculate_output_padding(self.expected_output_size, self.kernel_size[0], self.padding, self.stride, self.dilation)
            x = F.conv_transpose1d(x, self.weight, bias=self.bias, padding=self.padding, stride=self.stride, dilation=self.dilation, groups=self.groups, output_padding=op)
        return x
    ...
    def _create_weights_for_iterable_groups(self):
        ...
        for i in range(self._num_unique_groups):
            out_idx = torch.where(self._out_idx_map == i)[0]  # indices of the neurons in set i <--> indices of the output channels
            neuron_depth = val_unique[i]  # neurons in set i process val_unique[i] many channels each
            self.weight.append(nn.Parameter(neuron_depth, torch.empty(len(out_idx), *self.kernel_size, dtype=self.dtype)))
            self.bias = nn.Parameter(torch.empty(self.out_channels)) if self.use_bias else self.register_parameter('bias', None)
    ...
    def _process_iterable_groups(self, x):
        ...
        for i in range(self._num_unique_groups):
            ...
            out_idx = torch.where(self._out_idx_map == i)[0]  # indices of the neurons in set i <--> indices of the output channels
            inp = x[:, in_idx, :]
            out = F.conv_transpose1d(inp, self.weight[i], bias=self.bias[out_idx], padding=self.padding, stride=self.stride, dilation=self.dilation, groups=len(out_idx), output_padding=op)
            y.append(out)
            resort_idx.extend(out_idx.tolist())  # keep track of the order of the output channels, as we don't process them in order
        y = torch.cat(y, dim=1)
        resort_idx = torch.argsort(torch.tensor(resort_idx))
